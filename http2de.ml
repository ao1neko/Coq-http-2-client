...
type 's alignedEncodeM =
  (Buffer.t * in_channel) -> OCamlNativeInt.t -> 's -> cacheFormat ->
  (((Buffer.t * in_channel)*OCamlNativeInt.t)*cacheFormat) option
...
type 'a alignedDecodeM =
  (Buffer.t * in_channel) -> OCamlNativeInt.t -> cacheDecode ->
...
let rec setCurrentBytes cache0 cacheAddNat n sz =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      (fun _ ->
      if Nat.ltb idx n
      then Some ((((fun _ -> failwith "set_nth'" ) n v idx w),(Pervasives.succ
      ...
      if Nat.ltb idx n
      then let a =
             (((fun _ -> failwith "set_nth'" ) n v idx
...
(** val alignedEncodeList' :
    ...
    (Buffer.t * in_channel) -> OCamlNativeInt.t -> 'a1 list -> cacheFormat ->
    (((Buffer.t * in_channel)*OCamlNativeInt.t)*cacheFormat) option **)
...
(** val wrapDecoder :
    OCamlNativeInt.t -> (OCamlNativeInt.t -> (Buffer.t * in_channel) ->
    (('a1*OCamlNativeInt.t)*'a2) option) -> (Buffer.t * in_channel) -> 'a1 option **)
...
(** val wrapEncoder :
    OCamlNativeInt.t -> (OCamlNativeInt.t -> (Buffer.t * in_channel) -> 'a1 ->
    (((Buffer.t * in_channel)*OCamlNativeInt.t)*'a2) option) -> 'a1 ->
    (Buffer.t * in_channel) -> (Buffer.t * in_channel) option **)
...
(** val hTTP_Frame_encoder_impl :
    OCamlNativeInt.t -> (Buffer.t * in_channel) -> hTTP_Frame ->
    (((Buffer.t * in_channel)*OCamlNativeInt.t)*cacheFormat) option **)
...
let hTTP_Frame_encoder_impl sz v r0 =
  ...
  | Some a ->
    ...
    if Nat.ltb idx sz
    then let a0 =
           (((fun _ -> failwith "set_nth'" ) sz (let x,_ = let x,_ = a in x in x) idx
              r0.type0),(Pervasives.succ
         ...
         if Nat.ltb idx0 sz
         then let a1 =
                (((fun _ -> failwith "set_nth'" ) sz (let x,_ = let x,_ = a0 in x in x) idx0
                   r0.flags),(Pervasives.succ
              ...
              (match setCurrentBytes test_cache test_cache_add_nat sz (Pervasives.succ
               ...
               | Some a2 ->
                 (match alignedEncodeList test_cache (fun n v0 idx1 s ce ->
                          if Nat.ltb idx1 n
                          then Some
                                 ((((fun _ -> failwith "set_nth'" ) n v0 idx1 s),(Pervasives.succ
...
(** val hTTP_Frame_decoder_impl :
    OCamlNativeInt.t -> (Buffer.t * in_channel) ->
    ((hTTP_Frame*OCamlNativeInt.t)*cacheDecode) option **)

let hTTP_Frame_decoder_impl sz v =
  match let idx = 0 in
        (match (fun _ (buff,ic) n ->  let _ =  if Buffer.length buff > n then () else for i=Buffer.length buff to n do  Buffer.add_char buff (input_char ic)  done  in try let c = Char.code (Buffer.nth buff n ) in (*let _ =  Printf.printf "%d %x %c\n" n c (if 0x20 <= c && c <= 0x7e then Char.chr c else '.') in *)Some c with Invalid_argument _ -> None )
                 sz v idx with
         | Some a ->
...
(** val http_decode2 : OCamlNativeInt.t -> (Buffer.t * in_channel) -> hTTP_Frame option **)

let http_decode2 sz =
  wrapDecoder sz hTTP_Frame_decoder_impl

(** val http_encode2 :
    OCamlNativeInt.t -> hTTP_Frame -> (Buffer.t * in_channel) -> (Buffer.t * in_channel)
    option **)
